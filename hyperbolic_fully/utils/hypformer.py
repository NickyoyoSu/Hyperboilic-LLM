...
class TransConvLayer(nn.Module):
    def __init__(self, manifold, in_channels, out_channels, num_heads, use_weight=True, args=None):
        super().__init__()
        print(f"🚀 TransConvLayer initialized with num_heads = {num_heads}")  # 🔥 打印 num_heads
    ...
    def full_attention(self, qs, ks, vs, output_attn=False):
        ...
        mask = causal_mask(seq_len, att_weight.device)##TODO
...
class TransConv(nn.Module):
    ...
    def forward(self, x_input):
        ...
        for i, conv in enumerate(self.convs):
            #print(f"⚡ Before conv_{i}: x.shape = {x.shape}")  # 进入 conv 之前
            x = conv(x, x)
            #print(f"🔥 After conv_{i}: x.shape = {x.shape}")  # 经过 conv 之后

            # **🔥 打印 x 和 layer_[i] 的形状**
            #print(f"🔥 Layer {i}: x shape: {x.shape}, layer_[i] shape: {layer_[i].shape}")
            if layer_[i].shape[1] != x.shape[1]:  
                repeat_factor = x.shape[1] // layer_[i].shape[1]
                layer_[i] = layer_[i].repeat(1, repeat_factor, 1)  # **同步 num_heads**
            
            layer_.append(x)
            if self.residual:
                if x.shape != layer_[i].shape:
                    print(f"❌ Mismatch at layer {i}: x.shape = {x.shape}, layer_[i].shape = {layer_[i].shape}")
            ...
            # # x = self.dropout(x, training=self.training)
            layer_.append(x)
            #print(f"✅ layer_[{i}].shape = {layer_[i].shape}") 
...
class HypFormer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels,
        ...
        if self.decoder_type == 'euc':
            ...
            #################################改动#################################
            #################################改动#################################
            self.decode_trans = LorentzMLR(manifold=self.manifold_out, num_features=self.hidden_channels, num_classes=self.out_channels) 
