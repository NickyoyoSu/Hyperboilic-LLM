...
class LorentzBatchNorm2d(LorentzBatchNorm):
    ...
    def __init__(self, manifold_in: Lorentz, num_channels: int, manifold_out=None, space_method=False, momentum=0.1):
        if space_method:
            # 当space_method=True时，父类也使用减1后的通道数
            super(LorentzBatchNorm2d, self).__init__(manifold_in, num_channels-1, manifold_out)
            self.norm = nn.BatchNorm2d(num_features=num_channels-1, momentum=momentum)
        else:
    ...
    def forward(self, x, momentum=None):
        ...
        if self.space_method:
            ...
            # 正确索引时间分量
            x_space = x[:, 1:, :, :]   # 移除第一个通道（时间分量）
            normed_space = self.norm(x_space)
            x_time = ((normed_space ** 2).sum(dim=1, keepdim=True) + self.c).clamp_min(1e-6).sqrt()
            x = torch.cat([x_time, normed_space], dim=1)
            if self.manifold_out is not None:
                x = x * (self.manifold_out.c / self.c).sqrt()
            # 使用permute返回原始格式
            return x.permute(0, 2, 3, 1)  # [bs, c, h, w] -> [bs, h, w, c]
        x = x.view(bs, -1, c)
